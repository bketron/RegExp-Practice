...
function testEmail (email) {
    var reg = /\S*[\@]\S*[\.][A-z]+/
    var isEmail = Boolean(reg.exec(email))
...
function testMoney (str) {
    var reg = /^([\$](?=[0-9]))[0-9]*([\,]\d{3})?([\,]\d{3})?([\.]\d{2})?$/
    var isMoney = reg.test(str)
...
function markDownLink (text) {
    var reg = /^(\w+[\:]?[ ]?)?[\[](\S*[ \.]?\w+)[\]][\(](\S*)[\)]/gmi
    var newLink = text.replace(reg, function(match,p1,p2,p3){
        var newText = ''
...
function markDownItalics (text) {
    var reg = /(\*)(italic|partially|two|part italic|This text is italic.)(\*)/gmi
  var italicText = text.replace(reg, function(match,p1,p2,p3){
    p1 = '<em>'
    p3 = '</em>'
...
function extractEmails (text) {
    var reg = /\w*[\.\-]?\w*@\S*[\.][A-z]*/gmi
    var emails = text.match(reg)
